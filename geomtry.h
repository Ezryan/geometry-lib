...
double Polygon::Deg2Rad(double angle) { return angle * std::numbers::pi / 180; }
...
size_t Polygon::verticesCount() { return vertex_.size(); }

const std::vector<Point>& Polygon::getVertices() { return vertex_; }
...
bool Polygon::containsPoint(const Point& point) const {
  ...
  for (size_t i = 0; i < vertex_.size(); i++) {
    Point next_p = vertex_[(i + 1) % vertex_.size()];
    if (((vertex_[i].y <= point.y) && (next_p.y > point.y)) ||
        ((vertex_[i].y > point.y) && (next_p.y <= point.y))) {
      vt = (point.y - vertex_[i].y) / (next_p.y - vertex_[i].y);
      if (point.x < vertex_[i].x + vt * (next_p.x - vertex_[i].x)) {
        ++cn;
      }
    }
  }
...
class Ellipse: Shape {
 ...
 public:
  ...
  bool isCongruentTo(const Shape& another) const final;
  bool isSimilarTo(const Shape& another) const final;
  virtual bool containsPoint(const Point& point) const final;
...
double Ellipse::perimeter() const {
  return std::numbers::pi * (3 * (less_half_ + big_half_) -
         sqrt((3 * less_half_ + big_half_) * (less_half_ + 3 * big_half_)));
}

double Ellipse::area() const {
  return std::numbers::pi * less_half_ * big_half_;
}
...
double Circle::radius() { return radius_; }

double Circle::perimeter() const { return 2 * std::numbers::pi * radius_; }
...
double Rectangle::Rad2Deg(double ang) { return ang / std::numbers::pi * 180; }
...
Point Rectangle::center() { return {(fisrt_.x + second_.x) / 2, (fisrt_.y + second_.y) / 2}; }
...
Circle Square::circumscribedCircle() { return Circle(center(), side_ / sqrt(2)); }

Circle Square::inscribedCircle() { return Circle(center(), side_ / 2); }
...
Point Triangle::centroid() { return  {(getVertices()[0].x + getVertices()[1].x + getVertices()[2].x) / 3,
...
Line Triangle::EulerLine() { return {circumscribedCircle().center(), orthocenter()}; }
